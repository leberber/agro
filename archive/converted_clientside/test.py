...
def shop_card(product_name, product_code, product_price,  product_quantity, image_path):
    return dmc.Grid(
            id={"type": "grid_item", "index":product_code},
            className = 'card-grid',
            justify = 'center',
            children = [
                dmc.Col(
...
@callback(
        ...
        Output({"type": "card_item_quantity", "index": ALL}, "value"),
        Input({"type": "page_switcher_action", "index": ALL}, "n_clicks"),
        Input({"type": "x_out_from_cart", "index": ALL}, "n_clicks"),
        State({"type": "card_item_quantity", "index": ALL}, "value"),
...
def item_card_add_item(cart_click, x_click, card_item_quantity, items_pushed_to_cart):
    # for i in ctx.outputs_list:
    #     print(i)
    print(card_item_quantity)

    items_in_view = {}
    for index, item in enumerate(ctx.states_list[0]):
        item_quantity = item.get("value")
        item_id = item['id']['index']
        items_in_view[item_id] = index
        if item_quantity: 
    ...
    if ctx.triggered_id['type'] == 'x_out_from_cart':
        id_to_delete = ctx.triggered_id['index']
        del items_pushed_to_cart[id_to_delete]
        card_item_quantity[items_in_view[id_to_delete]] = 0
    cart_items = []
    print(card_item_quantity)
    ...
    return items_pushed_to_cart, html.Div(cart_items), card_item_quantity


    
# @app.callback(
#     Output('products_container', 'children', allow_duplicate=True),
#     State('products_container', 'children'),
#     Input({"type": "x_out_from_cart", "index": ALL}, "n_clicks"),
#     prevent_initial_call=True
# )

# def item_cssard_add_item(x, n ):
#     import json

#     # print(ctx.triggered_id['index'])
    
#     # x = x['props']['children'][0]['props']['children'][0]['props']['id']['index']
#     # print(len(x))
